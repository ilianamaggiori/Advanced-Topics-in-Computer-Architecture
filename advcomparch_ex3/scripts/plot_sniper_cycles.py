...
# Sort the tuples in results_tuples by the number of threads (n_threads)
for grain_size in results_tuples:
    results_tuples[grain_size] = sorted(results_tuples[grain_size], key=lambda x: x[1])
...
output_base = '/home/manolis/Desktop/graphs/sniper'
...
for (grain_size, res_tuples) in results_tuples.items():
    ...
    for tuple in tuples_by_lock_type:
        lock_type = tuple[0]
        nthread_axis = list(tuple[1][0])
        time_axis = list(tuple[1][1])
        x_ticks = 2**np.arange(0, len(time_axis))
        
        color = color_map.get(lock_type, default_color)
    ...
    output = os.path.join(output_base, 'grain-' + str(grain_size) + '.png')
    print("Saving: " + output)

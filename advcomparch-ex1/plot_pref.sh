...
for outFile in sys.argv[1:]:
	fp = open(outFile)
	line = fp.readline()
	ipc = None  # Define ipc variable outside the loop
	while line:
		tokens = line.split()
		if (line.startswith("Mean IPC:")):
			...
			lines_prefetched = sizeLine.split()[1]
	...
	#prefConfigStr = '{}'.format(lines_prefetched)
	#print (prefConfigStr)
	x_Axis.append(lines_prefetched)
	#print(lines_prefetched)
	#print (l2ConfigStr)
	#x_Axis.append(prefConfigStr)
	ipc_Axis.append(ipc)
...
ax1.grid(True)
ax1.set_xlabel("N lines for prefetch")
...
plt.savefig("pref.png",bbox_inches="tight")

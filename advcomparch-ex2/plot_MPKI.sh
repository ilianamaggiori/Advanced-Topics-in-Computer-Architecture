...
for outFile in sys.argv[1:]:
    with open(outFile, 'r') as fp:
        for line in fp:
            if line.startswith("For predictor"):
...
# Plot MPKI values
line1 = ax1.plot(xAx, mpki_Axis, label="direction MPKI", color="blue", marker='o')

# Set plot title and legend
plt.title("Mean direction MPKI for Different Predictors")
plt.legend()
plt.tight_layout()

# Save the plot to a file
plt.savefig("MPKI.png", bbox_inches="tight")
